...
The full code is avaalable from the GitHub repository linked at the foot of this post.
...
So let's collect the data from each of these media releases and visualise it as a time-series. Automated web-scraping was not used for these data because only a few data items need to be abstracted from each report, and the format of the daily reports evolved quite a lot -- sometimes it is easier to just capture the required data manually. 
