control.icm <- function(type, nsteps, nsims = 1, rec.rand = TRUE, quar.rand = TRUE, hosp.rand = TRUE, 
                        a.rand = TRUE, d.rand = TRUE, initialize.FUN = initialize.icm,
  ...
  if (is.null(p$type) | !(p$type %in% c("SI", "SIS", "SIR", "SEIR", "SEIQHR"))) {
    stop("Specify type as \"SI\", \"SIS\", \"SIR\", \"SEIR\", or \"SEIQHR\" ", call. = FALSE)
  }
  if (is.null(p$nsteps)) {
