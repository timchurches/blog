...
## SEIQHRF models - fixed vs time-variant act rates and inf probs
...
param <-  param.icm(inf.prob.i = 0.05, 
                    ...
                    dh.rate = (20/365)/1000,
                    dr.rate = (7/365)/1000)
t30_act_rate_seiqhrf_sim <- icm(param, init, control)

t30_act_rate_seiqhrf_sim_df <- as.data.frame(t30_act_rate_seiqhrf_sim, out="mean")
```
...
init <- init.icm(s.num = 997, i.num = 3, e.num=0, q.num=0, h.num=0, r.num = 0, f.num = 0)
param <-  param.icm(inf.prob.i = c(rep(0.05,30), rep(0.025, 335)), 
                    act.rate.i = 10, 
                    inf.prob.q = 0.05, 
                    act.rate.q = 5,                    
                    quar.rate = 1/10,
                    ...
                    dh.rate = (20/365)/1000,
                    dr.rate = (7/365)/1000)
t30_inf_prob_seiqhrf_sim <- icm(param, init, control)

t30_inf_prob_seiqhrf_sim_df <- as.data.frame(t30_inf_prob_seiqhrf_sim, out="mean")
```
...
param <-  param.icm(inf.prob.i = c(rep(0.05,30), rep(0.025, 335)), 
                    ...
                    dh.rate = (20/365)/1000,
                    dr.rate = (7/365)/1000)
t30_both_seiqhrf_sim <- icm(param, init, control)

t30_both_seiqhrf_sim_df <- as.data.frame(t30_both_seiqhrf_sim, out="mean")
```
...
param <-  param.icm(inf.prob.i = 0.05, 
                    ...
                    dh.rate = (20/365)/1000,
                    dr.rate = (7/365)/1000)
t15_act_rate_seiqhrf_sim <- icm(param, init, control)

t15_act_rate_seiqhrf_sim_df <- as.data.frame(t15_act_rate_seiqhrf_sim, out="mean")
```
...
init <- init.icm(s.num = 997, i.num = 3, e.num=0, q.num=0, h.num=0, r.num = 0, f.num = 0)
param <-  param.icm(inf.prob.i = c(rep(0.05,15), rep(0.025, 350)), 
                    act.rate.i = 10, 
                    inf.prob.q = 0.05, 
                    act.rate.q = 5,                    
                    quar.rate = 1/10,
                    ...
                    dh.rate = (20/365)/1000,
                    dr.rate = (7/365)/1000)
t15_inf_prob_seiqhrf_sim <- icm(param, init, control)

t15_inf_prob_seiqhrf_sim_df <- as.data.frame(t15_inf_prob_seiqhrf_sim, out="mean")
```
...
param <-  param.icm(inf.prob.i = c(rep(0.05,15), rep(0.025, 350)), 
                    ...
                    dh.rate = (20/365)/1000,
                    dr.rate = (7/365)/1000)
t15_both_seiqhrf_sim <- icm(param, init, control)

t15_both_seiqhrf_sim_df <- as.data.frame(t15_both_seiqhrf_sim, out="mean")
```
...
seiqhrf_sim_df %>%
  ...
  bind_rows(t30_inf_prob_seiqhrf_sim_df %>%
              select(time, s.num, e.num, i.num, q.num, 
                     h.num, r.num, f.num) %>%
              mutate(model = "t=30 inf prob")) %>%
  bind_rows(t30_act_rate_seiqhrf_sim_df %>%
              select(time, s.num, e.num, i.num, q.num, 
                     h.num, r.num, f.num) %>%
              mutate(model = "t=30 act rate")) %>%
  bind_rows(t30_both_seiqhrf_sim_df %>%
              select(time, s.num, e.num, i.num, q.num, 
                     h.num, r.num, f.num) %>%
              mutate(model = "t=30 both")) %>%
  bind_rows(t15_act_rate_seiqhrf_sim_df %>%
              select(time, s.num, e.num, i.num, q.num, 
                     h.num, r.num, f.num) %>%
              mutate(model = "t=15 act rate")) %>%
  bind_rows(t15_inf_prob_seiqhrf_sim_df %>%
              select(time, s.num, e.num, i.num, q.num, 
                     h.num, r.num, f.num) %>%
              mutate(model = "t=15 inf prob")) %>%
  bind_rows(t15_both_seiqhrf_sim_df %>%
              select(time, s.num, e.num, i.num, q.num, 
                     h.num, r.num, f.num) %>%
